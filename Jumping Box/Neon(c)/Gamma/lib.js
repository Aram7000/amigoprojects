...
let particles = (x, y, count, direction, size, speed, z = 0, parent = document.body, background = "#ffffff") => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        @keyframes particle-${i}${_GLOBAL_.particleGroupCount}-animation {
            0% {
                transform: rotateZ(0deg) translateZ(${z}px);
                opacity: 0;
                left: 0;
                top: 0;
            }
            ...
            100% {
                transform: rotateZ(${Math.random() * 360}deg) translateZ(${z}px);
                opacity: 0;
                left: ${left};
                top: ${top};
            }
